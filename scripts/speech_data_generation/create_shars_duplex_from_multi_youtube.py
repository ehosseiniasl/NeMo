...
def get_segments(input_string, speaker):
    try:
...
def merge_segments(segments):
    ...
    for i, s in enumerate(segments):
        # if i != 0 and i < len(segments) - 1 and s['speaker'] == new_segments[-1]['speaker']: #segments[i-1]['speaker']:
        if len(new_segments) > 0 and s['speaker'] == new_segments[-1]['speaker']:
            # new_segments.pop()
            # new_segments.append(
            #     {
            #         'start': segments[i-1]['start'],
            #         'end': s['end'],
            #         'words': segments[i-1]['words'] + s['words'],
            #         'speaker': s['speaker']
            #     }
            # )
            # new_segments[-1] = {
            #         # 'start': segments[i-1]['start'],
            #         'end': s['end'],
            #         'words': new_segments[-1]['words'].extend(s['words']),
            #         # 'speaker': s['speaker']
            #     }
            new_segments[-1]['end'] = s['end']
            new_segments[-1]['words'].extend(s['words'])
        else:
...
def is_valid(line):
    # if user is first
    user_segments = get_segments(line['user_pred_text'], speaker='user')
    agent_segments = get_segments(line['assistant_pred_text'], speaker='agent')
    
    if len(user_segments) == 0 or len(agent_segments) == 0:
        # ipdb.set_trace()
        return False
    return user_segments[0]['start'] < agent_segments[0]['start']
...
def create_shar_from_manifest(manifest, out_shar_dir, num_shard=10, dataset_name='squadv2', turn_silence_sec=0.32):
    ...
    print(f"num valid examples: {len(valid_manifest)}")
    
    shard_size = int(len(valid_manifest) / num_shard)
    if len(valid_manifest) % shard_size != 0:
        shard_size += 1
    print(f"shard_size {shard_size} num_shards {num_shard}")
    ...
    for j, cut in tqdm(enumerate(cuts)):
        ...
        user_segments = get_segments(valid_manifest[j]['user_pred_text'], speaker='user')
        ...
        agent_segments = get_segments(valid_manifest[j]['assistant_pred_text'], speaker='agent')
        ...
        #     )
        # ipdb.set_trace()
        all_segments.extend(agent_segments)
        all_segments.sort(key=lambda d: d['start'])
        merged_segments = merge_segments(all_segments)
        
        for segment in merged_segments:
            turn_start = segment['start'] * framerate
            turn_dur = (segment['end']-segment['start']) * framerate
            cut.supervisions.append(
                SupervisionSegment(
                    id=cut.id,
                    recording_id=cut.id,
                    start=turn_start, #segment['start'] * framerate, #total_dur,
                    duration=turn_dur, #(segment['end']-segment['start']) * framerate, #user_duration,
                    text="".join(segment['words']).strip(), #convs[i]["instruction"],
                    speaker=segment['speaker'], #convs[i]["from"],
                    language="EN",
                ),
            )
            # total_dur += turn_dur
        ...
        cut.duration = valid_manifest[j]['duration']
        # cut.start = 0.0
